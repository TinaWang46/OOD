...
class ResNet(nn.Module):
    ...
    class Head(nn.Module):
        ...
        def forward(self, x: Tensor) -> Tensor:
            if self.normalization is not None:
                x = self.normalization(x)
            x_features = self.activation(x)
            x_linear = self.linear(x_features)
            return x_linear, x_features
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.blocks(x)
        x_linear, x_features = self.head(x)
        return x_linear, x_features
...
class FTTransformer(nn.Module):
    ...
    def forward(self, x_num: Optional[Tensor], x_cat: Optional[Tensor]) -> Tensor:
        ...
        x = self.cls_token(x)
        x_linear, x_features = self.transformer(x)
        return x_linear, x_features
    
    def get_fc(self):
        fc = self.transformer.head.linear
        return fc.weight.cpu().detach().numpy(), fc.bias.cpu().detach().numpy(), fc
        
        

def apply_model(model, x_num, x_cat=None, return_features=False):
    if isinstance(model, FTTransformer):
        out_linear, out_features =  model(x_num, x_cat)
        if return_features:
            return out_linear, out_features
        else:
            return out_linear
            
    elif isinstance(model, (MLP, ResNet)):
        assert x_cat is None
        out_linear, out_features =  model(x_num)
        if return_features:
            return out_linear, out_features
        else:
            return out_linear
...
def get_params(architecture_name, d_out, lr, weight_decay, num_features):

    """
    Create a prediction model and optimizer based on the specified architecture.

    Args:
        architecture_name (str): The name of the architecture to use ('MLP', 'ResNet', or 'FTTransformer').
        d_out (int): The dimension of the output layer.
        lr (float): Learning rate for the optimizer.
        weight_decay (float): Weight decay for regularization.
        num_features (int): The number of input features.

    Returns:
        tuple: A tuple containing the created model and optimizer.

    Raises:
        Exception: If the specified architecture is not recognized.

    Example:
        >>> model, optimizer = get_params('MLP', d_out=10, lr=0.001, weight_decay=0.0001, num_features=64)
        >>> print(model)
        >>> print(optimizer)
    """
    ...
    if architecture_name == 'MLP':
    ...
    elif architecture_name == 'FTTransformer':
        ...
        raise Exception("Sorry, the architecture is not correct.")
    
    optimizer = (model.make_default_optimizer() if isinstance(model, FTTransformer)
